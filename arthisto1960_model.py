...
def convolution_block(input, filters:int, dropout:float=0, name:str=''):
    ...
    x = layers.SpatialDropout2D(rate=dropout, name=f'{name}_dropout')(x)
    return x

# Encoder block
def encoder_block(input, filters:int, dropout:float=0, name:str=''):
    x = convolution_block(input=input, filters=filters, dropout=dropout, name=name)
    p = layers.MaxPool2D(pool_size=(2, 2), name=f'{name}_pooling')(x)
    return x, p

# Decoder block
def decoder_block(input, skip, filters:int, dropout:float=0, name:str=''):
    ...
    x = convolution_block(input=x, filters=filters, dropout=dropout, name=name)
    return x

# Decoder block
def binary_unet(input_shape, filters):
    # Input
    inputs = layers.Input(input_shape)
    # Encoder path
    s1, p1 = encoder_block(input=inputs, filters=1*filters, dropout=0, name='encoder1')
    s2, p2 = encoder_block(input=p1,     filters=2*filters, dropout=0, name='encoder2')
    s3, p3 = encoder_block(input=p2,     filters=4*filters, dropout=0, name='encoder3')
    s4, p4 = encoder_block(input=p3,     filters=8*filters, dropout=0, name='encoder4')
    # Bottleneck
    b1 = convolution_block(input=p4, filters=16*filters, dropout=0, name='bottleneck')
    # Decoder path
    d1 = decoder_block(input=b1, skip=s4, filters=8*filters, dropout=0, name='decoder1')
    d2 = decoder_block(input=d1, skip=s3, filters=4*filters, dropout=0, name='decoder2')
    d3 = decoder_block(input=d2, skip=s2, filters=2*filters, dropout=0, name='decoder3')
    d4 = decoder_block(input=d3, skip=s1, filters=1*filters, dropout=0, name='decoder4')
    # Output
